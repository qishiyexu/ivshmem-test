...
// It's defined in ivshmem device 
...
static int ivpci_request_msix_vectors(struct ivpci_private *ivpci_dev, int n)
{
    ...
    for (i = 0; i < ivpci_dev->nvectors; i++) {
        ...
        // ivpci_dev->msix_names[i] shows in /proc/interrupt
        // ivpci_dev->msix_entries[i].vector was allocated by kernel, in `pci_enable_msix_exact`
        // ivpci_interrupt: irq handler
...
static struct pci_driver ivpci_driver = {
    .name       = DRV_NAME,
    // 用于设备和驱动匹配。platform总线platform_match的时候，有id_table优先用
    // id_table匹配，否则仅仅比较设备和驱动的name字段是否一致
...
static int __init ivpci_init(void)
{
    ...
    // 创建设备逻辑类，相当于在/sys/class下创建一个新的文件夹
    ...
    // 主设备号，0~233静态分配；234~254动态分配。linux/fs.h定义了动态分配的范围
    // alloc_chrdev_region 会优先从234~254动态分配一个，比如234.
